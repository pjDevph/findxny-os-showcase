...
interface Props {
  ...
  readonly data: ShiftReportData | null;
  readonly printing: boolean;
  readonly onClose: () => void;
  readonly onPrint: () => void;
}

export function ShiftCloseReportModal({ visible, data, printing, onClose, onPrint }: Props) {
  const { C } = useTheme();
  ...
  return (
    <ReportDetailModal
      visible={visible} title="SHIFT CLOSE REPORT" subtitle={`Shift #${data.shiftId.slice(-8)}`}
      onClose={onClose} closeLabel="Done"
      primaryAction={{ label: "Print", onPress: onPrint, busy: printing, kind: "info" }}
    >
      ...
      <View style={rs.rCard}>
        ...
        <ReportRow label="Expected" value={peso(data.expectedCash)} bold />
        <ReportRow label="Actual Cash" value={peso(data.actualCash)} />
        <ReportRow label="Variance" value={peso(data.variance)} bold />
        {data.variance < 0 && (
          <Text style={{ color: C.bad, fontSize: 12, fontWeight: "700", marginTop: 6 }}>
            ⚠ Shortage — actual cash is below expected.
          </Text>
        )}
      </View>
    </ReportDetailModal>
