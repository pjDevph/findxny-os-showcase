-- Extend create_pos_order to accept/store shift_id (added to orders in
-- 0111_orders_shift_id.sql) so a completed order can be attributed back to
-- the register/shift that rang it up — required for accurate per-shift
-- sales reporting and reconciliation (see pos-shift/index.ts's
-- handleGetShiftReport). Optional (NULL when the caller omits it), matching
-- every existing orders-create call exactly, same pattern as 0099's
-- ticket_no/payment_status addition.
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    ticket_no, payment_status, shift_id
  )
  VALUES (
