-- create_pos_order (staff POS) never needed ticket_no/payment_status at
-- creation time — those are set later by pos-counter-pay. The guest/kiosk
-- checkout path (public-orders-create) DOES need them at creation (a kiosk
-- ticket number, and payment_status='pending_counter' for pay-at-counter
-- orders), which is why it still does its own two-step insert instead of
-- going through this atomic, lock-ordered RPC like the staff path does.
--
-- Add both columns to the RPC (optional — NULL when the caller omits them,
-- matching every existing orders-create call exactly) so public-orders-create
-- can be switched onto the same atomic path in the next migration/deploy.
