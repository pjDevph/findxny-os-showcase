-- create_pos_order (from 0092_order_integrity_fixes.sql) never inserted
-- service_fee — it predates the service-charge persistence work that landed
-- separately on the old direct-insert code path (now merged into the same
-- atomic RPC as of this migration). CREATE OR REPLACE, same function
-- signature, only the orders INSERT column list changes.
