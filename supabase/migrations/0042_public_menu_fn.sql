-- Perf: collapse the public-menu edge function's 6 PostgREST round-trips
-- (workspace lookup + 5 parallel queries) into one server-side call that returns
-- the whole menu as a single JSON blob. Cuts the request waterfall to 1 round-trip.
--
-- Returns null when the slug doesn't resolve (edge fn maps that to 404).
-- Execute is restricted to service_role (the edge function calls it; not anon).
...
as $$
  ...
  select case when not exists (select 1 from ws) then null else jsonb_build_object(
    ...
    'products', coalesce((
      ...
      from products where workspace_id = (select id from ws) and active = true and for_sale = true), '[]'::jsonb),
